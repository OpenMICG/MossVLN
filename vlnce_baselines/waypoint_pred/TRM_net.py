...
class BinaryDistPredictor_TRM(nn.Module):
    ...
    def forward(self, rgb_feats, depth_feats, score):
        ...
        vis_rel_x = self.waypoint_TRM(
            vis_x, attention_mask=attention_mask, score=score
        ) # vis_rel_x:[bsi,12,768]
        vis_logits = self.vis_classifier(vis_rel_x) # vis_logits:[bsi,12,120]
        vis_logits = vis_logits.reshape(
            bsi, self.num_angles, self.n_classes) #vis_logits:[bsi,120,12]
