...
@baseline_registry.register_trainer(name="SS-Moss")
class RLTrainer(BaseVLNCETrainer):
    """定义了一个RLTrainer类，它接受一个可选的配置参数config。
    在初始化方法__init__中，调用了父类BaseVLNCETrainer的初始化方法，并将配置参数传递给父类进行初始化。
    同时，将配置参数中的IL.max_traj_len转换成整型，并赋值给self.max_len"""
    ...
    def _initialize_policy(
        ...
        start_iter = 0
        policy = baseline_registry.get_policy(self.config.MODEL.policy_name) # 这里的policy_name为CMAPolicy
        ...
        if load_from_ckpt:
            ...
            if 'module' in list(ckpt_dict['state_dict'].keys())[0] and self.config.GPU_NUMBERS == 1:
                ...
                self.policy.load_state_dict(ckpt_dict["state_dict"], strict=False)
            if config.IL.is_requeue:
                self.optimizer.load_state_dict(ckpt_dict["optim_state"], strict=False)
    ...
    def _nav_gmap_variable(self, cur_vp, cur_pos, cur_ori,obs):
        ...
        return {
            ...
            'no_vp_left': batch_no_vp_left, 'rgb_fts': batch_rgb_fts,
        }
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        for stepk in range(self.max_len):
            ...
            nav_inputs = self._nav_gmap_variable(cur_vp, cur_pos, cur_ori,wp_outputs)
            nav_inputs.update({
                'mode': 'navigation',
