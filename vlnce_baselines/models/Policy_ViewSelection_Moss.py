...
from vlnce_baselines import ss_trainer_Moss
import torch.nn.functional as F
from vlnce_baselines.models.moss.vlnbert_init import get_vlnbert_models
...
class PolicyViewSelectionMoss(ILPolicy):
    def __init__(
        self,
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, img_embeddings, txt_embeddings, label):
        distances = torch.norm(img_embeddings - txt_embeddings, p=2, dim=1)
...
class Moss(Net):
    ...
    def forward(self, mode=None, 
        ...
        if mode == 'language':
            ...
            txt_embeddings = observations['instruction']
            ...
            img_embeddings = rgb_embedding.view(rgb_embedding.size(0), -1)  # flatten the image embeddings

            device = img_embeddings.device
            txt_embeddings = txt_embeddings.to(device)
            img_dim = img_embeddings.size(1)

            linear_layer = nn.Linear(txt_embeddings.size(1), img_dim).to(device)
            txt_embeddings = linear_layer(txt_embeddings.float())


            if txt_embeddings.size(0) < img_embeddings.size(0):
                num_copies = img_embeddings.size(0) - txt_embeddings.size(0)
                last_sample = txt_embeddings[-1].unsqueeze(0).repeat(num_copies, 1)
                txt_embeddings = torch.cat([txt_embeddings, last_sample], dim=0)
            elif txt_embeddings.size(0) > img_embeddings.size(0):
                txt_embeddings = txt_embeddings[:img_embeddings.size(0)]

            img_embeddings = F.normalize(img_embeddings, dim=1)
            txt_embeddings = F.normalize(txt_embeddings, dim=1)
            labels = torch.arange(img_embeddings.size(0)).type_as(txt_embeddings).long()
            scores = img_embeddings.mm(txt_embeddings.t())
            scores /= 0.1
            scores1 = scores.transpose(0, 1)
            score0 = F.cross_entropy(scores, labels)
            score1 = F.cross_entropy(scores1, labels)
            score = score0 + score1

            ''' waypoint prediction ----------------------------- '''
            waypoint_heatmap_logits = waypoint_predictor(
                rgb_embedding, depth_embedding, score)
