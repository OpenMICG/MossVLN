...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    ...
    def forward_navigation(
        ...
        vp_embeds = F.interpolate(rgb_fts.unsqueeze(2), size=(gmap_embeds.size(1), gmap_embeds.size(2)), mode='nearest').squeeze(2)
        vp_embeds = vp_embeds.permute(0, 2, 1, 3).contiguous().view(vp_embeds.size(0), vp_embeds.size(2), -1)
        vp_embeds = vp_embeds[:, :, :768]
